...
from models.RigidNet_KshapeNet import RigidNet
from models.NonRigidNet_KshapeNet import NonRigidNet
...
data = np.load(r'C:\UI_PRMD_Dataset\data_and_features\action_classification_data\xyz\train_data.npy', allow_pickle=True)
labels = np.load(r'C:\UI_PRMD_Dataset\data_and_features\action_classification_data\xyz\train_label.pkl', allow_pickle=True)

data_2 = np.load(r'C:\UI_PRMD_Dataset\data_and_features\action_classification_data\ang\train_data.npy', allow_pickle=True)
labels_2 = np.load(r'C:\UI_PRMD_Dataset\data_and_features\action_classification_data\ang\train_label.pkl', allow_pickle=True)

labels = np.array(labels[1])
labels_2 = np.array(labels_2[1])

# ******* Using Position Only ********
train_data, test_data, train_labels, test_labels = train_test_split(data, labels, test_size=0.20, random_state=42, stratify=labels)
test_data, val_data, test_labels, val_labels = train_test_split(test_data, test_labels, test_size=0.4, random_state=42, stratify=test_labels)
...
for m in options:
    ...
    for r in range(runs):
        ...
        if rigid == True:
            model = RigidNet(mod=mod, run=r).to(device)
        else:
            model = NonRigidNet(mod=mod, run=r).to(device)
