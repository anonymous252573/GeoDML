...
for ex in range(1,7):
    ...
    folder_path = r'\EHE_Dataset\data_and_features\Extracted_Features\cv_cs\xyz\{}'.format(ex)  #path to dataset folder
    folds_acc = {}

    save_proba = r'C:\GeoDML\sigmoid\E{}'.format(ex)  # path to save the predicted probabilities from the test set
    ...
    for fold in range(len(os.listdir(folder_path))):
        ...
        for m in options:
            ...
            for r in range(runs):
                ...
                for dml_run in range(runs):
                    ...
                    model = model_class(mod=mod,run=r,dml_run=dml_run,num_labels=1,glob_h=True).to(device)
                    ...
                    with torch.no_grad():
                        ...
                        for i in range(steps + (1 if len(X_test) % batch_size_test != 0 else 0)):                 
                            ...
                            # ***** Get the output prediction probabilities for all the test samples ******
                            out_prob = torch.sigmoid(outputs.data)[:,0].clone().detach().cpu().numpy()
                            if end_idx > len(X_test):
                                test_proba[start_idx:] = out_prob
                            else:
                                test_proba[start_idx:end_idx] = out_prob
                            # print(torch.sigmoid(outputs.data)[:,0].shape)
        ...
        # Save the accuarcy, test prediction probabilities and attention weights for this fold in a dictionary.
