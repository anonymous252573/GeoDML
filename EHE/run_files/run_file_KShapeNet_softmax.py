...
for ex in range(1,7):
    ...
    parser.add_argument('--cast', default='log_0refseq', type=str,help = 'LogMap sameref (log_sref) or first frame of sequence (log_0refseq) or parallel transport (pt)')
    ...
    save_proba = r'C:\softmax\E{}'.format(ex)  # path to save the predicted probabilities from the test set
    ...
    for fold in range(len(os.listdir(folder_path))):
        ...
        for m in options:
            ...
            for r in range(runs):
                ...
                model = model_class(mod=mod, run=r).to(device)
        ...
        # Save the accuarcy and test prediction probabilities for this fold in a dictionary.
