...
data = np.load(r'C:\EHE_Dataset\data_and_features\action_classif_data\pos\train_data.npy', allow_pickle=True)
labels = np.load(r'C:\EHE_Dataset\data_and_features\action_classif_data\pos\train_label.pkl', allow_pickle=True)

# print(Counter(list(labels[1])))

data_2 = np.load(r'C:\EHE_Dataset\data_and_features\action_classif_data\ang\train_data.npy', allow_pickle=True)
labels_2 = np.load(r'C:\EHE_Dataset\data_and_features\action_classif_data\ang\train_label.pkl', allow_pickle=True)
...
test_data, val_data, test_labels, val_labels = train_test_split(test_data, test_labels, test_size=0.20, random_state=42, stratify=test_labels)
...
for m in options:
    ...
    for r in range(runs):
        ...
        for dml_run in range(runs+5):
            ...
            # opt = torch.optim.Adagrad(model.parameters(), lr=0.01, weight_decay=1e-2)
            # opt = torch.optim.RMSprop(model.parameters(), lr=learning_rate, alpha=0.2, weight_decay=1e-3, eps=1e-5)
            ...
            scheduler = ReduceLROnPlateau(opt, 
                                ...
                                patience=15,  
                                min_lr=1e-6)
            # scheduler = CosineAnnealingLR(opt, T_max=30, eta_min=1e-5)
