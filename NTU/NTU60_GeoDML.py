...
from models.RigidNet60_GeoDML import RigidNet60
from models.NonRigidNet60_GeoDML import NonRigidNet60
...
for m in options:
    ...
    for r in range(runs):
        ...
        for dml_run in range(runs):
            ...
            if rigid == True:
                model = RigidNet60(mod,num_frames,num_joints, r, dml_run=dml_run).to(device)
            else:
                model = NonRigidNet60(mod,num_frames,num_joints, r, dml_run=dml_run).to(device)
    ...
    transform_acc.append(max(run_acc))

    print('Finsished Running for {}'.format(m))
    print('Best accuracy for {} is {}'.format(mod, max(run_acc)))
